...
test.describe("Admin company registration", () => {
  ...
  test("registers a company and exposes it in the conversation selector", async ({ page }) => {
    ...
    await expect(page.getByText('Company "Aurum Logistics AG" registered with 1 users')).toBeVisible();
  ...
  test("blocks duplicate company registration", async ({ page }) => {
    ...
    await expect(page.getByText('Company "PAMP" already exists and cannot be registered twice.')).toBeVisible();
    await expect(page.getByRole("button", { name: "Register Company" })).toBeDisabled();
  });
});